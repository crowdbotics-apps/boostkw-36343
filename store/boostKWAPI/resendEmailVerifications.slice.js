...
const resendEmailVerificationsSlice = createSlice({
  ...
  reducers: {},
  extraReducers: {
    [accounts_registration_resend_email_create.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending"
      }
    },
    [accounts_registration_resend_email_create.fulfilled]: (state, action) => {
      if (state.api.loading === "pending") {
        state.entities.push(action.payload)
        state.api.loading = "idle"
      }
    },
    [accounts_registration_resend_email_create.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
