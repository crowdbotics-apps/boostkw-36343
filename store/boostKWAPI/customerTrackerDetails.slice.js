...
const customerTrackerDetailsSlice = createSlice({
  ...
  reducers: {},
  extraReducers: {
    [trackers_customer_tracker_inputs_read.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending"
      }
    },
    [trackers_customer_tracker_inputs_read.fulfilled]: (state, action) => {
    ...
    [trackers_customer_tracker_inputs_read.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
        state.api.error = action.error
        state.api.loading = "idle"
      }
    },
    [trackers_customer_tracker_inputs_update.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending"
      }
    },
    [trackers_customer_tracker_inputs_update.fulfilled]: (state, action) => {
      if (state.api.loading === "pending") {
    ...
    [trackers_customer_tracker_inputs_update.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
        state.api.error = action.error
        state.api.loading = "idle"
      }
    ...
    [trackers_customer_tracker_inputs_delete.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending"
      }
    },
    [trackers_customer_tracker_inputs_delete.fulfilled]: (state, action) => {
      if (state.api.loading === "pending") {
    ...
    [trackers_customer_tracker_inputs_delete.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
        state.api.error = action.error
        state.api.loading = "idle"
      }
