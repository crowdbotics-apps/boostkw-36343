...
const customerTrackerSerializerWithJobProcessesSlice = createSlice({
  ...
  reducers: {},
  extraReducers: {
    [trackers_customer_tracker_active_read.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending"
      }
    },
    [trackers_customer_tracker_active_read.fulfilled]: (state, action) => {
    ...
    [trackers_customer_tracker_active_read.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
        state.api.error = action.error
        state.api.loading = "idle"
      }
    },
    [trackers_customer_tracker_inputs_create.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending"
      }
    },
    [trackers_customer_tracker_inputs_create.fulfilled]: (state, action) => {
      if (state.api.loading === "pending") {
        state.entities.push(action.payload)
        state.api.loading = "idle"
      }
    },
    [trackers_customer_tracker_inputs_create.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
        state.api.error = action.error
        state.api.loading = "idle"
      }
